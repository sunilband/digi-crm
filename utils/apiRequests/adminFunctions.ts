...
export const getAllUsers = async (token:string) => {
    try {
      ...
      // @ts-ignore
      const response =await fetch(process.env.NEXT_PUBLIC_ALL_USERS, requestOptions)
...
export const getManagers = async (token:string) => {
  try {
    ...
    // @ts-ignore
    const response =await fetch(process.env.NEXT_PUBLIC_GET_MANAGERS, requestOptions)
    const responseData = response.json();
    return responseData
